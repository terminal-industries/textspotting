...
class DETECTION_TRANSFORMER(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        #self.input_proj = self.create_input_proj(cfg)            
    ...
    def forward(self, samples: NestedTensor):
        ...
        print(f'time resnet consume:{time_sum}')
        srcs = []
        masks = []
        time_start = time.time()    
        for l, feat in enumerate(features):
            src, mask = feat.decompose()
            proj_res = self.input_proj[l](src)
              
            srcs.append(proj_res)
            masks.append(mask)
            assert mask is not None
        #torch.cuda.synchronize()                
        time_sum = time.time() - time_start            
        print(f'time projection(2) consume:{time_sum}')            
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                pos.append(pos_l)
        '''                            
        src = self.input_proj[3](features[2].tensors)
        m = masks[0]
        mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]
        pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)
        srcs.append(src)
        ...
        #torch.cuda.synchronize()                
        time_sum = time.time() - time_start
        print(f'time transformer consume:{time_sum}')
        outputs_texts = []
        outputs_coords = []
        outputs_classes = []
        if self.boundary_head_on:
        ...
        #torch.cuda.synchronize()                
        time_sum = time.time() - time_start
        print(f'projection consumes:{time_sum}')
